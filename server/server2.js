...
const dbConfig = {
  ...
  database: "mitrdb", // Replace with your MySQL database name
};
...
app.put("/api/put/employee/employeeid/:employeeId", async (req, res) => {
  ...
  if (
    req.body.hasOwnProperty("profitCenter") &&
    req.body["profitCenter"] !== null
  ) {
    addUpdateValue("profitCenter", pool.escape(req.body["profitCenter"]));
  }
  if (req.body.hasOwnProperty("licenseId")) {
    const licenseIdValue =
      req.body["licenseId"] !== null ? parseInt(req.body["licenseId"]) : null;
...
app.put("/api/put/license/licenseid/:licenseId", async (req, res) => {
  ...
  // if (
  //   req.body.hasOwnProperty("expirationDate") &&
  ...
  if (req.body.hasOwnProperty("isActivate")) {
    const isActivateValue =
      req.body["isActivate"] !== null ? req.body["isActivate"] : null;
...
app.put("/api/put/hardware/hardwareid/:hardwareId", async (req, res) => {
  ...
  if (
    req.body.hasOwnProperty("manufacturer") &&
  ...
  if (
    req.body.hasOwnProperty("serialNumber") &&
  ...
  // if (
  //   req.body.hasOwnProperty("purchaseDate") &&
  //   req.body["purchaseDate"] !== null
  // ) {
  //   addUpdateValue("purchaseDate", pool.escape(req.body["purchaseDate"]));
  // }
  // if (
  //   req.body.hasOwnProperty("warrantyExpiration") &&
  //   req.body["warrantyExpiration"] !== null
  // ) {
  //   addUpdateValue(
  //     "warrantyExpiration",
  //     pool.escape(req.body["warrantyExpiration"])
  //   );
  // }
  if (req.body.hasOwnProperty("employeeOwnerId")) {
    const employeeOwnerIdValue =
      req.body["employeeOwnerId"] !== null
        ? parseInt(req.body["employeeOwnerId"])
        : null;
...
app.get("/api/get/hardware/hardwareid/:hardwareid", async (req, res) => {
  ...
  try {
    const query =
      "SELECT * FROM computer_hardware_devices WHERE hardwareId = ?";
    const [rows, fields] = await pool.query(query, [hardwareid]);
    res.json(rows);
  } catch (err) {
    console.log(err);
